...
export const AnimatedBackground: React.FC = () => {
      ...
      for (let i = 0; i < cols; i++) {
        for (let j = 0; j < rows; j++) {
          ...
          points.push({
            x,
            y,
            vx: (Math.random() - 0.5) * 1.5, // Velocidade aumentada
            vy: (Math.random() - 0.5) * 1.5, // Velocidade aumentada
            originalX: x,
            originalY: y,
          });
        }
  ...
  return (
    <canvas
      ...
      style={{
        background: 'linear-gradient(to bottom, #000000, #0a0520, #1a0a3a)',
        zIndex: 0,
      }}
    />
  );
